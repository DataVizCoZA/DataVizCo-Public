/* Order Flow Funnel — SAC custom widget (main component). Self-contained; no external libraries. */
...
/* =============================================================================
 * Order Flow Funnel — SAC main web component (v0.2)
 * -----------------------------------------------------------------------------
 * Data model:
 *   - Measures = stages. Feed order = left-to-right order. Drop in N measures.
 *   - Dimension (optional) = trellis. One panel per member (e.g. per DC/region).
 *   - Targets, in priority order per stage:
 *       1. a "target" measure paired to the stage by name (contains target/goal),
 *       2. the `targets` property (comma-separated list, one per stage in order),
 *       3. the `defaultTarget` property.
 * ========================================================================== */
...
(function () {
  ...
  template.innerHTML =
    ...
    '  .off-msg { width:100%; height:100%; display:flex; align-items:center; justify-content:center;' +
  ...
  function measName(m) { return m.description || m.label || m.id || ""; }
  function looksLikeTarget(m) {
    return /\b(target|goal)\b/i.test(measName(m));
  }
  function stem(name) {
    return String(name).toLowerCase().replace(/\b(target|goal|plan|actual)\b/g, "").replace(/[^a-z0-9]/g, "");
  }

  function buildPanels(dataBinding, props) {
    var data = dataBinding.data || [];
    var meta = parseMetadata(dataBinding.metadata);
    var dims = meta.dims, meas = meta.meas;

    // split measures into stages (feed order) and target measures
    var stageM = [], targetM = [];
    meas.forEach(function (m) { (looksLikeTarget(m) ? targetM : stageM).push(m); });
    if (!stageM.length) { stageM = meas.slice(); targetM = []; } // all looked like targets? treat all as stages

    // pair each stage to a target measure by name stem
    var pairKeyFor = stageM.map(function (sm) {
      var smStem = stem(measName(sm)), hit = null;
      targetM.forEach(function (tm) { if (!hit && stem(measName(tm)) === smStem) hit = tm.key; });
      return hit;
    });

    // explicit per-stage targets list from the styling panel
    var targetsList = String(props.targets || "").split(",")
      .map(function (t) { return t.trim(); }).filter(function (t) { return t !== ""; })
      .map(Number);

    function targetFor(i, row) {
      if (pairKeyFor[i] && row[pairKeyFor[i]]) return toNum(row[pairKeyFor[i]].raw);
      if (targetsList.length > i && isFinite(targetsList[i])) return targetsList[i];
      return toNum(props.defaultTarget);
    }

    function stagesFromRow(row) {
      return stageM.map(function (m, i) {
        return {
          name: measName(m),
          actual: toNum(row[m.key] && row[m.key].raw),
          target: targetFor(i, row)
        };
      });
    }

    var panels = [];
    if (dims.length >= 1 && data.length) {           // trellis: one panel per member
      var dimKey = dims[0].key;
      data.forEach(function (row) {
        var cell = row[dimKey] || {};
        panels.push({ title: cell.label != null ? cell.label : (cell.id || ""), stages: stagesFromRow(row) });
      });
    } else if (data.length) {                        // single panel
      panels.push({ title: null, stages: stagesFromRow(data[0]) });
    }

    // percentage scaling across every actual value
    var mode = props.valueScale || "auto";
    var maxActual = 0;
    panels.forEach(function (p) { p.stages.forEach(function (s) { if (s.actual > maxActual) maxActual = s.actual; }); });
    var scaleFraction = mode === "fraction" || (mode === "auto" && maxActual > 0 && maxActual <= 1.5);
    if (scaleFraction) {
      panels.forEach(function (p) {
        p.stages.forEach(function (s) { s.actual *= 100; if (s.target <= 1.5) s.target *= 100; });
      });
    }
    return panels;
  }

  function rendererOptions(props) {
    return {
      title: props.title,
      subtitle: props.subtitle,
      goodColor: props.goodColor,
      warnColor: props.warnColor,
      badColor: props.badColor,
      amberThreshold: Number(props.amberThreshold),
      redThreshold: Number(props.redThreshold),
      showTargetGhost: props.showTargetGhost !== false,
      showLegend: props.showLegend !== false,
      decimalSeparator: props.decimalSeparator || ","
    };
  }

  class OrderFlowFunnel extends HTMLElement {
    constructor() {
      super();
    ...
    render() {
      ...
      if (!binding || binding.state !== "success") {
        this._root.innerHTML = '<div class="off-msg">Add one measure per stage. Add a dimension to trellis the flow by member.</div>';
        return;
      }
      var panels = buildPanels(binding, this._props);
      this._root.innerHTML = buildOrderFlowSVG(panels, rendererOptions(this._props));
    }
  }

  customElements.define("com-dvc-sac-orderflowfunnel", OrderFlowFunnel);
})();
